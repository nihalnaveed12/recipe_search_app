...
export default function RecipeSearch() {
    ...
    const [searched , setSearched] = useState<boolean>(false)
     ...
     const handleSearch = async (event:FormEvent<HTMLFormElement>) => {
        ...
        setRecipes([])
        try {
            const response = await fetch(`https://api.edamam.com/search?q=${query}&app_id=${process.env.NEXT_PUBLIC_EDAMAM_APP_ID}&app_key=${process.env.NEXT_PUBLIC_EDAMAM_APP_KEY}`)
    ...
    return (
            <div className="flex flex-col h-full w-full max-w-6xl mx-auto p-4 md:p-6 bg-zinc-100 shadow-md mt-2">
